const PreviewUtils = {
  ...
  async openPreview({ 
    ...
    try {
      const dataUrl = await this.renderPdfPage(resultBlob, idx, 2.0)
      if (dataUrl) {
  ...
  async openPreviewByIndex({ 
    ...
    try {
      const dataUrl = await this.renderPdfPage(resultBlob, index, 2.0)
      vm.currentPreviewImage = dataUrl || ''
    } catch (error) {
      console.warn('Preview error:', error)
  ...
  async openPreviewFirstPage({ 
    ...
    try {
      const dataUrl = await this.renderPdfPage(resultBlob, 0, 2.0)
